...
function dy = cart_pendulum_dynamics(y, m, M, L, g,  u)
  ...
  dy(1,1) =y(2) ;
  dy(2,1) =(u+(m*sin(y(3)-pi))*((L*y(4)*y(4))-g*cos(y(3)-pi)))/(M+(m*sin(y(3)-pi)*sin(y(3)-pi)));
  dy(3,1) = y(4);
  dy(4,1) = ((-u*cos(y(3)-pi))+((M+m)*(g)*sin(y(3)-pi))-(m*L*y(4)*y(4)*sin(y(3)-pi)*cos(y(3)-pi)))/(L*(M+m*sin(y(3)-pi)*sin(y(3)-pi)));  
endfunction
...
function [t,y] = pole_place_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  #eigs=[-sqrt(g/L); -sqrt(g/L) ;-sqrt(g/L); -sqrt(g/L)];
  eigs=[-1; -2; -3; -4];
  #eigs=[0 ;-sqrt(((M-m)*g)/(M*L)); 0; -sqrt(((M-m)*g)/(M*L))];
  K = place(A,B,eigs);
  disp(K);
  tspan = 0:0.1:10;
...
function [t,y] = lqr_cart_pendulum(m, M, L, g, y_setpoint, y0)
  ...
  Q =[2 0 0 0;0 2 0 0;0 0 2 0;0 0 0 2] ;                   ## Initialise Q matrix
  R = 1;                   ## Initialise R 
